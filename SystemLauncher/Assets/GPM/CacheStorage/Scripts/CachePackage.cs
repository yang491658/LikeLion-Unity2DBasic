...
namespace Gpm.CacheStorage
{
    ...
    public class CachePackage
    {
        ...
        public CacheRequestOperation Request(string url, CacheRequestConfiguration config, RequestCallback onResult)
        {
            ...
            else if (info.CanUseCache(config) == false)
            {
                useCache = false;    
            }
            else if (info.NeedRequest(config) == false)
            {
                if (IsValidCacheData(info) == true)
                {
