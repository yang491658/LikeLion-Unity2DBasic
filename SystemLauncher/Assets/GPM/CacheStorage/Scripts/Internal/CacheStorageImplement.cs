namespace Gpm.CacheStorage.Internal
{
    ...
    internal static class CacheStorageImplement
    {
        ...
        internal static CacheRequestOperation Request(string url, CacheRequestConfiguration config, Action<GpmCacheResult> onResult)
        {
            if (Initialized == false)
            {
                GpmLogger.Error("Not initialized", NAME, typeof(CacheStorageImplement));
            ...
            return cachePackage.Request(url, config, onResult);
        }
        ...
        internal static CacheRequestOperation RequestHttpCache(string url, Action<GpmCacheResult> onResult)
        {
            ...
            return Request(url, new CacheRequestConfiguration(CacheRequestType.ALWAYS, 0), onResult);
        }
        ...
        internal static CacheRequestOperation RequestTexture(string url, CacheRequestConfiguration config, bool preLoad, Action<CachedTexture> onResult)
        {
            if (Initialized == false)
            {
                GpmLogger.Error("Not initialized", NAME, typeof(CacheStorageImplement));
            ...
            if (cachedInfo != null)
            {
                ...
                bool needRequest = cachedInfo.NeedRequest(config);
            ...
            return Request(url, config, (result) =>
            {
                if (result.IsSuccess() == true)
                {
