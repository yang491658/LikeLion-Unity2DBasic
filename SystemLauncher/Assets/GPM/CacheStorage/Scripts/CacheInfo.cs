...
namespace Gpm.CacheStorage
{
    ...
    public class CacheInfo : IComparable<CacheInfo>
    {
        ...
        public bool CanUseCache(CacheRequestConfiguration config)
        {
            if (responseTime > 0)
            {
                if (CheckOverTime(config.validCacheTime.max) == true)
                {
                    return false;
                }    
            }
        ...
        public bool NeedRequest(CacheRequestConfiguration config)
        {
            if (config.requestType == CacheRequestType.LOCAL)
            {
                return false;
            }
            
            if (CheckLessTime(config.validCacheTime.min) == true)
            {
                return false;
            }
            
            if (config.requestType == CacheRequestType.ALWAYS)
            {
                return true;
            }
            else if (config.requestType == CacheRequestType.FIRSTPLAY)
            {
            ...
            if (CheckOverTime(config.reRequestTime) == true)
            {
                return true;
            }
        ...
        public bool CheckLessTime(double time)
        {
            if (time > 0 &&
                responseTime > 0)
            {
                return DateTime.UtcNow.Ticks - responseTime < TimeSpan.TicksPerSecond * time;
            }

            return false;
        }
        
        public bool CheckOverTime(double time)
        {
            if (time > 0 &&
                responseTime > 0)
            {
