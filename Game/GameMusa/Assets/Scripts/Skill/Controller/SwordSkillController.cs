...
public class SwordSkillController : MonoBehaviour
{
    ...
    // 스핀 설정 함수
    public void SetSpin(float _spinDistance, float _spinDuration, bool _isSpinning, float _hitCooldown)
    {
    ...
    private void DamageBySword(Enemy enemy)
    {
        ...
        // 시간 정지 코루틴
        enemy.StartCoroutine("FreezeTimeCoruntine", freezeTimeDuration);

        // 플레이어 장신구
        EquipmentData amulet = Inventory.instance.GetEquipmentType(EquipmentType.Amulet);
        if (amulet != null) // 플레이어 장신구 있음
        {
            amulet.DoItemEffect(enemy.transform); // 장신구 아이템 효과 실행
        }
    }
